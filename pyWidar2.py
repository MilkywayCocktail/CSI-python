...
if __name__ == "__main__":
    ...
    csi = MyCsiW2(conf, '0726A00', '../npsave/0726/0726A00-csio.npy')
    csi.load_data(remove_sm=True)
    #csi.load_lists()
    #csi.load_label('../sense/0725/01_labels.csv')
    #csi.remove_csd()
    csi.extract_dynamic(mode='overall-divide', ref='tx', ref_antenna=0, subtract_mean=False)
    csi.extract_dynamic(mode='highpass', cutoff=10)
    #csi.slice_by_label(overwrite=True)
    widar = MyWidar2(conf, csi)
    widar.run(pick_tx=1, dynamic_durations=False)
    widar.plot_results()
